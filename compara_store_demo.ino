...
void loop() {
  ...
  if (buttonValue>=1000){
    // If right button is pressed, the static demo version will appear
    Serial.println("Right Button Pressed");
    demo_mode = true;
    // Set counter display to 5 and set scale to show '<'
    counter = 5;
    display_scale_result(1, false);
    updateCounter();
    delay(500);
  }
  //For 2nd button:
  ...
  // If not in demo mode, run code for compara-store game
  if (!demo_mode) {
